...
{
	[Condition("VSConditions/CheckVSMachine")]
	[Help("Checks whether two booleans have the same value")]
	public class VSCheckExecuter: ConditionBase
		//where T : VSCheckExecuter<T>
    {
        [InParam("Machine")] ScriptMachine _machine;
		protected IMachine Machine => _machine.GetReference().machine;//{ get; }
		...
		// Ending task status
		private bool response = false, generated = false;

		// Actions
		private EventHook resumeHook;
		private EventHook suspendHook;

		private EventHook onCompletedCheckHook;

		private EventHook setOutTaskArgsHook;

		private EventHook onCheckHook;

        // Main class method, invoked by the execution engine.

        public void OnFisrtNeeded()
        {
            ...
            onCompletedCheckHook = new EventHook(Constants.EventTriggers.CompleteCheck, Machine);
            EventBus.Register<bool>(onCompletedCheckHook, OnCheckCompleted);

            /// EVENTS FROM THE EXECUTER TO THE VS
            onCheckHook = new EventHook(Constants.EventTriggers.onBBCheck, Machine);
        }
		...
		private void OnCheckCompleted(bool check_)
		{
			if (!generated)
            {
				OnFisrtNeeded();
